...
int main() {
  YAML::Node cfg =
      YAML::LoadFile(THIS_COM "config/towr_plus/draco3_stair.yaml");
  Clock clock = Clock();
  double time_solving(0.);

  // Locomotion Task
  LocomotionTask task = LocomotionTask("draco3_stair");
  task.from_yaml(cfg["locomotion_task"]);

  // Locomotion Solution
  LocomotionSolution sol =
      LocomotionSolution("draco3_stair", cfg["locomotion_param"]);
