...
Draco3CompositeRigidBodyInertia::Draco3CompositeRigidBodyInertia()
    : CompositeRigidBodyInertia(draco3_crbi_helper_n_in(), 3) {

  draco3_crbi_helper_work(&f_sz_arg_, &f_sz_res_, &f_sz_iw_, &f_sz_w_);
  jac_draco3_crbi_helper_work(&jac_f_sz_arg_, &jac_f_sz_res_, &jac_f_sz_iw_,
                              &jac_f_sz_w_);
  ...
  jac_f_x_ = new double *[jac_draco3_crbi_helper_n_in()];
  for (int i = 0; i < jac_draco3_crbi_helper_n_in(); ++i) {
    if (i == jac_draco3_crbi_helper_n_in() - 1) {
  ...
  jac_f_y_ = new double *[jac_draco3_crbi_helper_n_out()];
  for (int i(0); i < jac_draco3_crbi_helper_n_out(); ++i)
...
Eigen::MatrixXd
Draco3CompositeRigidBodyInertia::ComputeInertia(const Eigen::Vector3d &base_pos,
...
Eigen::MatrixXd Draco3CompositeRigidBodyInertia::ComputeDerivativeWrtInput(
  ...
  jac_draco3_crbi_helper(const_cast<const double **>(jac_f_x_), jac_f_y_,
                         jac_f_iw, jac_f_w, jac_draco3_crbi_helper_checkout());
